...
//        $createList = '0123456789';
        ...
        $partnerTradeNo = $orderNumber ?: ('ZOULUDUODUO' . time() . substr($nonceStr, 1, 5));

        $data = array(
            'amount' => $amount * 100, //单位是分
//            'amount' => $amount , //单位是分
            'check_name' => 'NO_CHECK',//NO_CHECK：不校验真实姓名 FORCE_CHECK：强校验真实姓名
            'desc' => '走路多多提现',//企业付款备注，必填。注意：备注中的敏感词会被转成字符*
            'mch_appid' => 'wx1f7e4716b2ad0a78',//申请商户号的appid或商户号绑定的appid
            'mchid' => '1601011621',//微信支付分配的商户号
...
//            're_user_name' => ''//如果check_name设置为FORCE_CHECK，则必填用户真实姓名
            'spbill_create_ip' => '101.10.10.10'//该IP同在商户平台设置的IP白名单中的IP没有关联，该IP可传用户端或者服务端的IP。
        );
        $strArr = array();
        foreach ($data as $key => $value) {
            $strArr[] = $key . '=' . $value;
        }
        $strArr[] = 'key=87ba8ee9c5f9c76f9eadccc029570111';
